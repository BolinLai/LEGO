...
We also release our detailed acton descriptions curated from GPT-3.5 in files under `curation_from_gpt`, which are used for VLLM instruction tuning.  **Note: They are only necessary if you want to finetune the VLLM component in LEGO. Otherwise, you can directly use the released [VLLM weights](#model-weights) for inference.**
