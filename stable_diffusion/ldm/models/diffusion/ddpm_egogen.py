...
class DDPM(pl.LightningModule):
    ...
    def sample(self, batch_size=16, return_intermediates=False):
        ...
        return self.p_sample_loop((batch_size, channels, image_size, round(image_size * self.aspect_ratio)), return_intermediates=return_intermediates)
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.projection_img = nn.Linear(5120, 768)
        self.projection_txt = nn.Linear(5120, 768)
        self_attn = nn.TransformerEncoderLayer(d_model=768, nhead=8, batch_first=True, norm_first=True)
        self.self_attn_mix = nn.TransformerEncoder(encoder_layer=self_attn, num_layers=2)
    ...
    def get_learned_conditioning(self, c):
        if self.cond_stage_forward is None:  # = None
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        with torch.no_grad():
            ...
            if bs is not None:
                x = x[:bs]  # very important to deal with the last batch
            x = x.to(self.device)
            encoder_posterior = self.encode_first_stage(x)
            z = self.get_first_stage_encoding(encoder_posterior).detach()
            ...
            if bs is not None:  # = None
                xc["c_crossattn"] = xc["c_crossattn"][:bs]  # very important to deal with the last batch
                xc["c_concat"] = xc["c_concat"][:bs]  # very important to deal with the last batch
            cond = {}
            ...
            if 'prior_mask' not in xc:
                cond["c_concat"] = [input_mask * self.encode_first_stage((xc["c_concat"].to(self.device))).mode().detach()]
            else:
                if bs is not None:
                    xc['prior_mask'] = xc['prior_mask'][:bs]  # very important to deal with the last batch
                cond["c_concat"] = [input_mask * torch.cat([self.encode_first_stage((xc["c_concat"].to(self.device))).mode().detach(), xc['prior_mask'].to(self.device)], dim=1)]

        # Add features from llava
        if "c_add_crossattn" in xc.keys():            
            if len(xc["c_add_crossattn"].keys()) == 2 and 'llava_image' in xc["c_add_crossattn"].keys() and 'llava_text' in xc["c_add_crossattn"].keys():
                if bs is not None:  # = None
                    xc["c_add_crossattn"]['llava_image'] = xc["c_add_crossattn"]['llava_image'][:bs]  # very important to deal with the last batch
                    xc["c_add_crossattn"]['llava_text'] = xc["c_add_crossattn"]['llava_text'][:bs]  # very important to deal with the last batch

                add_crossattn_img = self.projection_img(xc["c_add_crossattn"]['llava_image'])
                add_crossattn_txt = self.projection_txt(xc["c_add_crossattn"]['llava_text'])
                add_crossattn_txt = self.self_attn_mix(add_crossattn_txt)

                add_crossattn_img = (1 - prompt_mask.long()) * add_crossattn_img
                add_crossattn_txt = (1 - prompt_mask.long()) * add_crossattn_txt
                cond["c_crossattn"].append(add_crossattn_img)
                cond["c_crossattn"].append(add_crossattn_txt)
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start))  # (32, 4, 32, 32)
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)  # (32, 4, 32, 32)
    ...
    def sample(self, cond, batch_size=16, return_intermediates=False, x_T=None,
        ...
        if shape is None:
            shape = (batch_size, self.channels, self.image_size, round(self.image_size * self.aspect_ratio))
        if cond is not None:
            if isinstance(cond, dict):
                cond = {key: cond[key][:batch_size] if not isinstance(cond[key], list) else
                list(map(lambda x: x[:batch_size], cond[key])) for key in cond}
    ...
    def sample_log(self,cond,batch_size,ddim, ddim_steps,**kwargs):
        ...
        if ddim:
            ...
            shape = (self.channels, self.image_size, round(self.image_size * self.aspect_ratio))
    ...
    def log_images(self, batch, N=4, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        if plot_progressive_rows:
            with self.ema_scope("Plotting Progressives"):
                img, progressives = self.progressive_denoising(c, shape=(self.channels, self.image_size, round(self.image_size * self.aspect_ratio)), batch_size=N)
