...
def compute_statistics_of_path(path, model, batch_size, dims, device, num_workers=1):
    if path.endswith('.npz'):
        ...
        files = list()
        for clip_id in os.listdir(path):
            for action_id in os.listdir(os.path.join(path, clip_id)):
                images = sorted(os.listdir(os.path.join(path, clip_id, action_id)))
                for im in images:
                    files.append(os.path.join(path, clip_id, action_id, im))
        files.sort()
...
def main(paths, batch_size=1, num_workers=None, device='cuda', dims=2048, save_stats=False):
    ...
    fid_value = calculate_fid_given_paths(paths=paths, batch_size=batch_size, device=device, dims=dims, num_workers=num_workers)
    # print('FID: ', fid_value)
    return fid_value
